    ...
    try:
            ...
            try:
                ...
                if df_filtered.empty and total <= 0:
                    zero_shops.append({
                        ...
                        '区分': category_lookup.get(norm(shop_name), ""),
                ...
                if not df_filtered.empty:
                    ...
                    df_filtered['区分']      = category_lookup.get(norm(shop_name), "")
                    df_filtered['sort_date'] = pd.to_datetime(df_filtered[col_name_date], errors='coerce')

                    res_df = df_filtered[['ファイル名', '区分', '店舗名', col_name_date, col_name_bento, 'sort_date']]
                    res_df.columns = ['ファイル名', '区分', '店舗名', '日付', '弁当注文人数', 'sort_date']
        ...
        if combined_list or zero_shops or missing_shops:
            ...
            if combined_list:
                ...
                final_df = pd.DataFrame(columns=['【注文あり店舗】レク費url', 'ファイル名', '区分', '店舗名', '日付', '弁当注文人数'])
